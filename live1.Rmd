...
saida2_runjags %>% 
  runjags:::plot.runjags(vars = "alpha_A_L", plot.type = c("density"))
saida2_runjags %>% 
  runjags:::plot.runjags(vars = "lambda_A_L", plot.type = c("density"))
```
...
saida2_runjags %>% 
  runjags:::plot.runjags(vars = "beta_A_B", plot.type = c("density"))
saida2_runjags %>% 
  runjags:::plot.runjags(vars = "lambda_B_L", plot.type = c("density"))
```
...
if (file.exists("dataset3.Rds")) {
  ...
  dataset3 %>% 
    ...
    mutate(
      ...
      width_pixels = ScreenResolution %>% str_extract("([0-9])*x"),
      height_pixels = ScreenResolution %>% str_extract("x([0-9]*)"),
      Touchscreen = ScreenResolution %>% str_detect(fixed("Touchscreen"))
    ) %>% 
...
***AQUI:*** modelo de regressão multi-nível
...
- com Estatística Clássica, tenta-se resolver atraves de modelagem de equações estruturais (SEM)
- com Estatística Bayesiano, é um modelo hierárquico multinível
